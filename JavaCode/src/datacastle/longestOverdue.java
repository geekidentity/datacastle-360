...
public class longestOverdue {
	...
	public static void main(String[] argv) throws IOException {
		...
		// GBPerBank(NewdataPath+"Good_New_bill_detail.txt",
		// NewdataPath+"Good_perBillBank.txt");
		// GBPerBank(NewdataPath+"Bad_New_bill_detail.txt",
		// NewdataPath+"Bad_perBillBank.txt");
		// GBPerBank(trainDataPath+"clean_bill_detail_train.txt",
		// trainDataPath+"Train_perBillBank.txt");
		// GBPerBank(testDataPath+"clean_bill_detail_test.txt",
		// testDataPath+"Test_perBillBank.txt");
	...
	public static HashMap<Long, String> timeFeature(String billFile, String timeFeatureFile) throws IOException {
		...
		while (iter.hasNext()) {
			...
			// long maxInterval = finishTime.get(finishTime.size()-1) -
			// startTime.get(0);
			// long minInterval = finishTime.get(0) -
			// startTime.get(startTime.size()-1); // 最大、最小时间间隔

			/*
			 * 空账户标志、空账户数量、空账户比例、一行多卡标志、最多持卡数、总共持卡数量、最早的开卡时间、已知开卡时间、最短卡龄、最长卡龄
			 */
			String timeString = deadCardFlag + "," + blankAccount + "," + blankAccountRatio + "," + cardsPerBankFlags
					+ "," + cardsPerBankCount.get(cardsPerBankCount.size() - 1) + "," + cardsTotal + "," +
					// (startTime.get(startTime.size()-1) < 0 ?
					// "NA":startTime.get(startTime.size()-1))+","+
					// (finishTime.get(finishTime.size()-1) < 0 ?
					// "NA":finishTime.get(finishTime.size()-1))+","+
					startTime.get(startTime.size() - 1) + "," + finishTime.get(finishTime.size() - 1) + ","
					+ differTime.get(0) + "," + differTime.get(differTime.size() - 1) + ","
