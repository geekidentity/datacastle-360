...
class bankFeature2 {
	...
	/*
	 * public long commerceInterval = 0L; // 用户交易平均间隔 public long inputInterval
	 * = 0L; // 平均收入、支出时间间隔，以及之差 public long outputInterval = 0L; public long
	 * ioIntervalDiffer = 0L; public int ioFrequency = 0; // 收入频繁还是支出频繁?
	 */
...
public class bankRecord2 {
	...
	public static bankFeature2 getFeatureList(ArrayList<bankDetail> detailList) // 获取用户的银行记录特征
	{
		...
		while (it.hasNext()) {
			...
			} else if (temp_bankDetail.commerceType == 0) // 0代表收入
			{
				...
				if (temp_bankDetail.salaryLabel == 1) {
					;// result.salarySum += temp_bankDetail.commerceSum; //
						// 工资总收入 ，标记1为工资收入
						// result.salaryCount ++;
		...
		// result.salaryAvg = result.salarySum == 0 ? 0 : result.salarySum /
		// result.salaryCount; // 平均工资水平
		 ...
		 * result.ioIntervalDiffer = result.inputInterval -
		 * result.outputInterval; result.ioFrequency = result.ioIntervalDiffer
		 * <= 0 ? 1 : 0; // 收入间隔短为1，支出间隔短为0
		 */
	...
	/*
	 * public static HashMap<Long,Integer> label(String labelFile)throws
	 * IOException { HashMap<Long,Integer> userLabel = new
	 * HashMap<Long,Integer>();
	 * 
	 * File file=new File(labelFile); BufferedReader reader = null; reader = new
	 * BufferedReader(new FileReader(file));
	 * 
	 * String tempString = null; String regex = "\\s+";
	 * 
	 * while( (tempString = reader.readLine())!=null) { String str =
	 * tempString.replaceAll(regex, " "); String [] temp = str.split(",");
	 * userLabel.put(Long.valueOf(temp[0]), Integer.valueOf(temp[1])); } return
	 * userLabel; } public static HashMap<Long,Integer> label0(String
	 * labelFile)throws IOException { HashMap<Long,Integer> userLabel = new
	 * HashMap<Long,Integer>();
	 * 
	 * File file=new File(labelFile); BufferedReader reader = null; reader = new
	 * BufferedReader(new FileReader(file));
	 ...
	 * while( (tempString = reader.readLine())!=null) { String str =
	 * tempString.replaceAll(regex, " "); String [] temp = str.split(",");
	 * if(Integer.valueOf(temp[1]) == 0) userLabel.put(Long.valueOf(temp[0]),
	 * Integer.valueOf(temp[1])); } return userLabel; } public static
	 * HashMap<Long,Integer> label1(String labelFile)throws IOException {
	 ...
	 * while( (tempString = reader.readLine())!=null) { String str =
	 * tempString.replaceAll(regex, " "); String [] temp = str.split(",");
	 * if(Integer.valueOf(temp[1]) == 1) userLabel.put(Long.valueOf(temp[0]),
	 * Integer.valueOf(temp[1])); } return userLabel; } public static void
	 * labelRecord(HashMap<Long,Integer> userLabel,String originFile,String
	 * labeledFile)throws IOException { File file=new File(originFile);
	 * BufferedReader reader = null; reader = new BufferedReader(new
	 * FileReader(file)); String tempString = null;
	 * 
	 * File file2=new File(labeledFile); FileWriter fw = null; BufferedWriter
	 * writer = null; fw = new FileWriter(file2); writer = new
	 * BufferedWriter(fw);
	 * 
	 * while((tempString = reader.readLine())!=null) { String [] temp =
	 * tempString.split(","); if(userLabel.get(Long.valueOf(temp[0])) != null) {
