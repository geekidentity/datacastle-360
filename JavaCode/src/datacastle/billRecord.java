...
class billFeature { // 共计 维特征
	...
	// 目前主要在这个维度上做文章！！！
	/*
	 * 还款意愿 ： 还款多过欠款(还款能力强)：期数、期数占比、金额总数、金额占比 轻度拖欠：期数、金额
	 * 重度拖欠：期数、期数占比、金额总数、金额占欠款总数的比例 拖欠笔数： 轻度拖欠笔数 + 重度拖笔数 // 共性向量
	 */
	...
	public String combineFetureTitle() {
		...
		// 还款意愿模型 ：11
		title += "moreCount,moreRatio,moreSum,moreSumRatio," + "lightCount,lightSum,"
				+ "heavyCount,heavyRatio,heavySum,heavySumRatio," + "delayCount,";

		// 逾期模型 ： 6
		title += "curOverdueCount,curOverdueCountRatio," + "curOverdueMoney,curOverdueMoneySum,"
	...
	public String toString() {
		...
		// 欠款模型：14
		title += totalOwing + "," + totalBacking + "," + totalBackSubOwe + "," + totalBackSubOweFlag + ","
				+ perBankOwing + "," + perTermOwing + "," + owingBankMax + "," + owingPerMax + "," + withoutOweCount
				+ "," + withoutOweRatio + "," + owingBanks + "," + owingBanksRatio + "," + balanceNegativeCount + ","
				+ balanceNegativeRatio + ",";
		...
		title += loanUseTime + "," + perOwingAfterLoaning + "," + withoutOwingBeforeLoan + "," + backRatio + ","
				+ banksMoreThanFour + ",";
...
public class billRecord {
			...
			throws IOException // 一个用户的各个银行下的账单清单
	{
		...
		resultBill.moreBacking[3] = resultBill.totalOwing <= 0 ? -1
				: (float) resultBill.moreBacking[2] / resultBill.totalOwing;

		resultBill.heavyOwing[1] = (float) resultBill.heavyOwing[0] / recordCount; // 重度拖欠期数，金额占总欠款的占比
		resultBill.heavyOwing[3] = resultBill.totalOwing <= 0 ? -1
				: (float) resultBill.heavyOwing[2] / resultBill.totalOwing;
		...
		// resultBill.creditImproveFlag = resultBill.creditImproveFlag >
		// bankCount * 0.7 ? 1 : 0; // 超过 70% 的银行信用下降，则整体信用下降

		/* 消费模型 */
		resultBill.consumeAverage = resultBill.consumeSumCount / bankCount; // 平均每个银行下的消费笔数
		resultBill.consumePerCost = resultBill.consumeSumCount == 0 ? -1
				: resultBill.consumeSumMoney / resultBill.consumeSumCount; // 平均每笔的消费水平
		...
		/* 贷款前后的请情况对比 */
		resultBill.beforeLoanBill[4] = resultBill.beforeLoanBill[0] <= 0 ? 10f
				: (resultBill.beforeLoanBill[0] - resultBill.beforeLoanBill[1]) / resultBill.beforeLoanBill[0];

		resultBill.afterLoanBill[4] = resultBill.afterLoanBill[0] <= 0 ? 10f
				: (resultBill.afterLoanBill[0] - resultBill.afterLoanBill[1]) / resultBill.afterLoanBill[0];

		resultBill.loanUseTime = resultBill.billTimeList.get(resultBill.billTimeList.size() - 1)
				- loanTimeMap.get(Long.valueOf(billList.get(0).userID)) <= 0 ? -1
						: resultBill.billTimeList.get(resultBill.billTimeList.size() - 1)
								- loanTimeMap.get(Long.valueOf(billList.get(0).userID));
		// 贷款使用时间 (最后一笔账单时间 - 放款时间 )

		// 贷款之后平均每个月欠银行多少钱
		resultBill.perOwingAfterLoaning = resultBill.loanUseTime == -1 ? -1
				: resultBill.totalBackSubOwe / resultBill.loanUseTime * 30f;
		...
		resultBill.banksMoreThanFour = resultBill.bankCount >= 4 ? 1 : 0; // 银行数目
																			// >=4?
	...
	public static billFeature getPerBank(int bankId, ArrayList<BillDetail> billList, HashMap<Long, Long> loanTimeMap)
		...
		// ArrayList<Long> heavyTimeStampList = new ArrayList<Long> (); //
		// 重度拖欠时间戳清单
		...
		for (int i = 0; i < billList.size(); i++) {
			...
			if (eachDetail.lastBill != 0 && eachDetail.lastReturn != 0
					&& eachDetail.lastBill <= eachDetail.lastReturn) {
		 ...
		 * = billOfThisBank.creditYearsShortest = 30L; else
		 * if(timeSeries.get(timeSeries.size()-1) == 0) // 时间戳全是未知
		 * billOfThisBank.creditYearsAverage = billOfThisBank.creditYearsLongest
		 * = billOfThisBank.creditYearsShortest = timeSeries.size()*30L; //
		 * 按照每月30天计算 else { int zeroNotZero = 0;
		 * for(;zeroNotZero<timeSeries.size();zeroNotZero++) // 确知的最早的账单时间 {
		 * if(timeSeries.get(zeroNotZero) != 0) break; } if(zeroNotZero <=
		 * timeSeries.size()/2) // 至少一半的账单带有时间戳
		 * billOfThisBank.creditYearsAverage = billOfThisBank.creditYearsLongest
		 * = billOfThisBank.creditYearsShortest =
		 * timeSeries.get(timeSeries.size()-1) - timeSeries.get(zeroNotZero);
		 * else // 缺失的时间戳太多 billOfThisBank.creditYearsAverage =
		 * billOfThisBank.creditYearsLongest =
		 * billOfThisBank.creditYearsShortest = timeSeries.size()*30L; }
		 */
		// billOfThisBank.creditYearsAverage = billOfThisBank.creditYearsLongest
		// = billOfThisBank.creditYearsShortest
		...
		if (timeSeries.get(timeSeries.size() - 1) == 0)
			billOfThisBank.creditYearsAverage = billOfThisBank.creditYearsLongest = billOfThisBank.creditYearsShortest = timeSeries
					.size() * 30L; // 按照每月30天计算
		else
			billOfThisBank.creditYearsAverage = billOfThisBank.creditYearsLongest = billOfThisBank.creditYearsShortest = timeSeries
					.get(timeSeries.size() - 1) - timeSeries.get(0); // =
																		// timeSeries.size()*30L;
		return billOfThisBank;
	}

	public static float maxAbsElement(ArrayList<Float> list) // 绝对值最大的数，正负号有含义的，用户欠银行
																// 还是 银行欠用户
	{
		if (list.size() == 1)
			return list.get(0);
