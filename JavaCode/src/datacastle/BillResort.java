...
public class BillResort {
	...
	public static ArrayList<billDetail> cleanAndfill(ArrayList<billDetail> billList) {
		...
		// 非零时间戳与未知时间戳混合账单
			...
			/*
			 * Iterator<billDetail> it1 = ZeroBill.iterator(); //
			 * 先去重未知时间戳的重复账单，但是重复账单仍然有作用的 while(it1.hasNext()) { billDetail zero
			 * = it1.next(); for(billDetail Notzero : NotZeroBill) { //
			 * 判定重复账单的条件 if(zero.lastBill == Notzero.lastBill && zero.lastReturn
			 * == Notzero.lastReturn && zero.credit == Notzero.credit &&
			 * zero.curLeastReturn == Notzero.curLeastReturn &&
			 * zero.thisBillRest == Notzero.thisBillRest) { Notzero.consume =
			 * Math.max(zero.consume,Notzero.consume); // 消费笔数进行修正 break; } } }
			 */
