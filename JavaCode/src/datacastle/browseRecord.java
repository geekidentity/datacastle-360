...
class browseFeature {
	...
	// 全局 总浏览数/浏览次数/平均每次浏览个数
	// topK总浏览数/每次topK/topK占浏览的比重/与else的比重，其他选项/其他选项平均次数，浏览平均时间间隔/
	...
	public String toString() {
		...
		title += firstTime + "," + timeBeforeLoan + "," + countBeforeLoan + "," + timeRecentBeforeLoan + ","
				+ avgInterval + ",";
		return title;
	}
...
public class browseRecord {
	public static float AvgInterval(ArrayList<Long> timeArray) {
		...
		for (int i = 0; i < timeArray.size() - 1; i++) {
			if (timeArray.get(i + 1) != timeArray.get(i) && timeArray.get(i) != 0) // 未知时间戳不参与计算
																					// //两次同类型浏览行为的平均间隔
			{
			...
			String distributionFile, String loanTimeFile, int topKCount) throws IOException // 获取浏览行为记录特征
	{
		...
		ArrayList<String> mainBehaviors = topKBrowser(distributionFile, topKCount); // 主要的topKCount个浏览行为对(统计后大约31个
																					// 10万+)
		...
		while (it2.hasNext()) {
			...
			userBrowseMap.get(user).browse[4] = userBrowseMap.get(user).browse[3]
					/ userBrowseMap.get(user).topKtimeList.size();

			userBrowseMap.get(user).browse[8] = userBrowseMap.get(user).elsetimeList.size() == 0 ? 0
					: userBrowseMap.get(user).browse[7] / userBrowseMap.get(user).elsetimeList.size();

			userBrowseMap.get(user).browse[5] = userBrowseMap.get(user).browse[3] / userBrowseMap.get(user).browse[0];
			userBrowseMap.get(user).browse[6] = userBrowseMap.get(user).browse[7] == 0 ? -1
					: userBrowseMap.get(user).browse[3] / userBrowseMap.get(user).browse[7];
			...
			// 放款之前最近的一次浏览行为发生的时间
			if (beforeLoanArray.size() != 0)
				userBrowseMap.get(user).timeRecentBeforeLoan = loanTimeMap.get(user)
						- beforeLoanArray.get(beforeLoanArray.size() - 1);
			else
	...
	/*
	 * public static browseFeture getFetureList(ArrayList<browseHistory>
	 * detailList,ArrayList<String> mainBehaviors,int topKCount,int subCount) {
	 * //获取用户的浏览历史特征 : 在TopK和其他 主+次 浏览行为、11种子行为上的分布, browseFeture result = new
	 * browseFeture(topKCount,subCount); int topKBrowseTimes = 0;
	 * 
	 * HashSet<Long> totalTimes = new HashSet<Long>(); HashSet<Long> differTime
	 * = new HashSet<Long>(); //浏览topK记录的次数
	 * 
	 * Iterator<browseHistory> it = detailList.iterator(); while(it.hasNext()) {
	 * browseHistory temp_browseDetail = it.next();
	 * totalTimes.add(temp_browseDetail.time); String behavior =
	 * temp_browseDetail.browseMain+"	"+ temp_browseDetail.browseChild;
	 * //主、次浏览
	 * 
	 * int topindex; if((topindex = topIndex(mainBehaviors,behavior)) != -1)
	 * //第k个主浏览行为次数累加 { differTime.add(temp_browseDetail.time);
	 * result.topKDistribution[topindex]++; //topK浏览记录对应次数累加 topKBrowseTimes++;
	 * } else result.elseDistribution++;
	 * result.subBehavior[temp_browseDetail.browseChild-1]++; }
	 * result.browserPerTime = (totalTimes.size() == 0) ? 0 :
	 * detailList.size()/totalTimes.size(); //平均每次浏览记录数
	 * result.browserTopKPerTime = (differTime.size() == 0) ? 0 :
	 * topKBrowseTimes/differTime.size(); //平均每次浏览topK记录的次数 return result; }
	 */
	public static int topIndex(ArrayList<String> mainBehaviors, String topString) // 对应哪个
																					// topK
																					// 浏览记录和子记录
	{
